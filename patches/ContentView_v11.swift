        ...
        NavigationStack {
            VStack(spacing:8) {
                if model.discData.isEmpty {
                    ...
                    Text("يتم نسخ الملف مؤقتًا من Files حتى يمكن قراءة CTJ.bin داخل مجلد Gamma.").font(.caption).foregroundStyle(.secondary).multilineTextAlignment(.center).padding(.horizontal)
                    Spacer()
                } else {
                    HStack {
            ...
            ScrollView {
                LazyVGrid(columns:columns,spacing:16) {
                    ForEach(0..<0x2D,id:\.self) { i in
                        VStack {
                            Image(String(format:"%02X",i)).interpolation(.none).resizable().scaledToFit().frame(width:72,height:60).background(.black.opacity(0.3)).clipShape(RoundedRectangle(cornerRadius:10))
                            Text("TEAM \(String(format:"%02X",i))").font(.caption.bold())
                        }
                    }
                }.padding()
            }
            .navigationTitle("Team Logos 00–2C").navigationBarTitleDisplayMode(.inline)
        }
    }
}
