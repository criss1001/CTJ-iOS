...
extension CTJModel {
    func playerPortrait(team:Int, player:Int) -> UIImage? {
        guard !discData.isEmpty else { return nil }
        do {
            let disc = try PSXDiscImage(data: discData)
            let wanted = String(format:"TEAM%02X.BIN",team)
            guard let file = disc.files.first(where:{ !$0.isDirectory && $0.name.uppercased()==wanted }) else { return nil }
            let teamBytes = [UInt8](try disc.readFile(file))
            guard let top = CTJPortraitDecoder.offsetTable(teamBytes), top.count > 0,
...
enum CTJPortraitDecoder {
    static func u32(_ d:[UInt8],_ p:Int)->Int { Int(d[p]) | Int(d[p+1])<<8 | Int(d[p+2])<<16 | Int(d[p+3])<<24 }
    static func offsetTable(_ d:[UInt8]) -> [[UInt8]]? {
        ...
        var offs=[Int](); for i in 0..<count { let o=u32(d,4+i*4); guard o<=d.count else{return nil}; offs.append(o) }
    ...
    static func decompress(_ src:[UInt8]) throws -> [UInt8] {
        guard src.count>=5 else{throw NSError(domain:"CTJ",code:1)}; let target=(Int(src[2])<<8|Int(src[3]))+1; var out=[UInt8](); var pos=4
        while out.count<target { guard pos<src.count else{throw NSError(domain:"CTJ",code:2)}; var flags=src[pos];pos+=1
            for _ in 0..<8 { if out.count>=target{break}; guard pos<src.count else{throw NSError(domain:"CTJ",code:3)}; let b1=src[pos];pos+=1
                if flags&1 != 0 { out.append(b1) } else { guard pos<src.count else{throw NSError(domain:"CTJ",code:4)}; let b2=src[pos];pos+=1; let length=Int(b1&0x0F)+3; let distance=Int(b1>>4)|(Int(b2)<<4); var sp=out.count-distance; for _ in 0..<length { if out.count>=target{break}; out.append(sp<0 ? 0:out[sp]);sp+=1 } }
                flags >>= 1
            }
        }; return out
    }
